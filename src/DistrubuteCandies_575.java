 ...
 * 
 */
public class DistrubuteCandies_575 {

	public static int distributeCandies(int[] candies) {
		// 利用HashSet集合中元素的不重复性
		Set<Integer> eleCollec = new HashSet<Integer>();
		for (int i : candies) {
			eleCollec.add(i);
		}
		return eleCollec.size() >= candies.length / 2 ? candies.length / 2 : eleCollec.size();   //若不重复元素的个数大于糖果数量的一半，则不能保证兄弟和妹妹分得同样数量的糖果
	}
